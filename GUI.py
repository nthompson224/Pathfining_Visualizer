...
class SortFrame(QWidget):
    ...
    def initUI(self):
        ...
        self.selection_sort.clicked.connect(
            lambda: self.button_checked(self.selection_sort))
        self.bubble_sort.clicked.connect(
            lambda: self.button_checked(self.bubble_sort))
        self.merge_sort.clicked.connect(
            lambda: self.button_checked(self.merge_sort))
        self.quick_sort.clicked.connect(
            lambda: self.button_checked(self.quick_sort))
        self.heap_sort.clicked.connect(
            lambda: self.button_checked(self.heap_sort))
        self.cocktail_sort.clicked.connect(
            lambda: self.button_checked(self.cocktail_sort))
        self.gnome_sort.clicked.connect(
            lambda: self.button_checked(self.gnome_sort))
    ...
    def create_graph(self):
        ...
        self.view = self.plot.addViewBox(
            border='#45A5FF', enableMouse=False, enableMenu=False)

        self.plot.addLabel('Select an Algorithm', 1, 0,
                           size='16pt', color='#6BC7FF')
    ...
    def reset_arr(self):
        ...
        y = [randint(1, 500) for i in range(self.arr_size)]
        self.sort_graph.setOpts(
            height=y, brushes=[self.graph_colors[index] for index in range(self.arr_size)])
...
>>>>>>> ffdf361efe7a5d0f79d9b5bc1ddda3ab46d93b15
        ...
        if self.selection_sort.isChecked():
            ...
            worker = Worker(self.sort.merge_sort_helper_fn,
                            arr, 0, len(arr), len(arr))
    ...
    def sort_complete(self):
        ...
        self.sort_graph.setOpts(brushes=[colors[index]
                                for index in range(self.arr_size)])
        self.size_slider.blockSignals(False)
        self.sort_bttn.blockSignals(False)
<<<<<<< HEAD
        self.reset_bttn.blockSignals(False)
